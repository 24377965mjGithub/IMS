        ...
        <div class="col-lg-10 col-md-10 col-sm-12 main">
            <div class="container-fluid">
                @if (request()->get('search'))
                    <h1 class="page-title-h1">Customers</h1>
                    <p class="py-3">
                        <b><a href="{{ url('/customers') }}" style="color: blue; text-decoration: none;">Show all Customers</a> | Showing results for "{{ request()->get('search') }}".</b>
                    </p>
                @else
                    <h1 class="page-title-h1">Customers</h1>
                @endif
                ...
                <div class="main-box">
                    <div class="table-heading">
                        ...
                        @if (session('customerAdded'))
                            <p class="alert alert-success">{{ session('customerAdded') }}</p>
                        @endif
                        @if (session('customerUpdated'))
                            <p class="alert alert-success">{{ session('customerUpdated') }}</p>
                        @endif
                        @if (session('customerDeleted'))
                            <p class="alert alert-success">{{ session('customerDeleted') }}</p>
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-12 buttons">
                                ...
                                <button class="btn btn-primary new-btn" data-toggle="modal" data-target="#customer">New Customer</button>
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ url('/addcustomer') }}" method="post">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="customersName" placeholder="Customer Name..." class="form-control">
                                                </div>
                                                <div class="form-group">
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-striped">
                            <thead>
                                <tr>
                                  <th scope="col">
                                    <input type="checkbox" name="" id="" class="checkallcustomers">
                                    <i class="fa fa-trash delCustomer"></i>
                                  </th>
                              ...
                              <tbody>
                                @foreach ($customers as $customer)
                                    <tr>
                                        <th scope="row">
                                            <input type="checkbox" name="" id="" class="checkcustomers" data-id="{{ $customer->id }}">
                                        </th>
                                        <td scope="col">{{ $customer->customersName }}</td>
                                        @foreach ($customerTypes as $customerType)
                                            @if ($customer->customersType == $customerType->id)
                                                ...
                                                @else
                                                    ...
                                                    <td scope="col">{{ $customerType->customersType }}</td>
                                                @endif
                                            @endif
                                        @endforeach
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <a class="dropdown-item" href="#" data-toggle="modal" data-target="#edit{{ $customer->id }}">Edit</a>
                                                <a class="dropdown-item" href="#" data-toggle="modal" data-target="#delete{{ $customer->id }}">Delete</a>
                                            </div>
                                            ...
                                            <div class="modal fade" id="edit{{ $customer->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                  <div class="modal-content">
                                                    <div class="modal-header">
                                                      <h5 class="modal-title" id="exampleModalLabel">Edit <b>{{ $customer->customersName }}?</b></h5>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form action="{{ url('/saveeditcustomer', ['id' => $customer->id]) }}" method="post">
                                                            @csrf
                                                            <div class="form-group">
                                                                <label for="">Customer Name</label>
                                                                <input type="text" name="customersName" placeholder="Customer Name..." class="form-control" value="{{ $customer->customersName }}">
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="">Customer Type</label>
                                                                <select name="customersType" id=""  class="form-control">
                                                                    @foreach ($customerTypes as $customerType)
                                                                        @if ($customer->customersType == $customerType->id)
                                                                            <option value="{{ $customerType->id }}" selected>{{ $customerType->customersType }}</option>
                                                                        @else
                                                                            <option value="{{ $customerType->id }}">{{ $customerType->customersType }}</option>
                                                                        @endif
                                                                    @endforeach
                                            ...
                                            <div class="modal fade" id="delete{{ $customer->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                  <div class="modal-content">
                                                    <div class="modal-header">
                                                      <h5 class="modal-title" id="exampleModalLabel">Delete <b>{{ $customer->customersName }}?</b></h5>
                                                    </div>
                                                    <div class="modal-body">
                                                        <p>
                                                            Do you want to remove <b>{{ $customer->customersName }}</b>? All data associated with this customer will be lost. This action cannot be undone.
                                                        </p>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <a href="{{ url('/deletecustomer', ['id' => $customer->id]) }}">
